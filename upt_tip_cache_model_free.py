...
class UPT(nn.Module):
    ...
    def __init__(self,
        ...
        if self.class_nums==117 :text_inputs = torch.cat([clip.tokenize(verb) for verb in hico_verbs]) # hico_verbs 'action is ' +
            ...
            text_inputs = torch.cat([clip.tokenize(self.old_prompt2new_prompt(hico_text_label[id])) for id in hico_text_label.keys()])
        ...
        self.branch = 'text_only' ## F_cluster, F_cluster_semantic_label, F_vis, F_vis_semantic_label, text_only
                                  ## F_cluster_F_vis_semantic_label
        self.temperature = 1
        self.topk = topk
        num_shot = 32
        iou_rank = False
        self.use_outliers = False ## priority: use_outliers > kmeans > iou_rank
        ...
        print('>>> use_outliers:', self.use_outliers)
        ...
        cluster_file = 'clusters_80_117.p'
        anno_cluster = pickle.load(open(cluster_file,'rb'))
    ...
    def old_prompt2new_prompt(self, prompt):
        ...
        verb = prompt.split(' ')[-3]
        if 'ing' in verb:
            # new_prompt = f'{verb} a {obj}'
            # new_prompt = f'you are a person, and you are {verb} a {obj}'
            new_prefix = f'interaction between a person and a {obj}, '
            # ## remove old prefix
            prompt = ' '.join(prompt.split()[3:])
            new_prompt = new_prefix + prompt
        else:
            new_prompt = f'no interaction between a person and a {obj}'
            ...
            for i, hum_emb, obj_emb, embeddings in zip(indexes, hum_embeddings, obj_embeddings, union_embeddings):
                ...
                if len(range_lens) >= K_shot:
                    ...
                    if self.use_outliers:
                        sample_embeddings = self.select_outliers(torch.as_tensor(np.array(embeddings)).float(), K=K_shot)
                        sample_obj_embeddings = self.select_outliers(torch.as_tensor(np.array(obj_emb)).float(), K=K_shot)
                        sample_hum_embeddings = self.select_outliers(torch.as_tensor(np.array(hum_emb)).float(), K=K_shot)
                else:
                    lens = len(embeddings)

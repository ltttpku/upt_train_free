...
class Adapter(nn.Module):
    def __init__(self,
        ...
        self.mhsa = TransformerDecoderLayer(self.down_size, 4, self.down_size*4,
                                                self.dropout, 'relu', False)
        # self.mhsa = TransformerDecoderLayer_womhsa(self.down_size, 4, self.down_size*4,
        #                                         self.dropout, 'gelu', False) # current all experiments under this setting
        # self.mhsa = TransformerDecoderLayer_womhsa(self.down_size, 4, self.down_size*4,
    ...
    def forward(self, x, add_residual=True, residual=None, prior=None):
        ...
        if prior is not None:
            ...
            down = self.mhsa.forward_post(down, context, tgt_mask=None,
                           memory_mask=None,
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, adapter: bool=False):
        ...
        # pdb.set_trace()
        if adapter:
            self.adaptermlp = Adapter(None,  d_model=768 , dropout=0., bottleneck=64,
                                    init_option='lora',
                                    adapter_scalar='0.1',
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, adapter: bool=False):
        ...
        self.width = width
        self.layers = layers
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, attn_mask, adapter) for _ in range(layers)])
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        self.transformer = Transformer(width, layers, heads, adapter=True)
